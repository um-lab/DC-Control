...
class MultiControlNetUnionModel(ModelMixin):
    ...
    def forward(
        ...
        # down_block_res_samples_list, mid_block_res_sample_list = zip(*results)
        # down_block_res_samples_list, mid_block_res_sample_list = list(down_block_res_samples_list), list(mid_block_res_sample_list)
        # down_block_res_samples_list = [list(item) for item in zip(*down_block_res_samples_list)]
...
class StableDiffusionXLControlNetUnionPipeline(
    ...
    def __call__(
        ...
        if not isinstance(control_guidance_start, list) and isinstance(control_guidance_end, list):
            ...
            # mult = 2 if isinstance(controlnet, MultiControlNetUnionModel) else 1
            ...
            if isinstance(image, torch.Tensor):
                ...
                for img in image:
                    if isinstance(img, torch.Tensor):
                        original_size = original_size or img.shape[-2:]
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if isinstance(controlnet, ControlNetModel_Union):
                    ...
                    for union_control_type_ in union_control_type:
                        added_cond_kwargs.append({"text_embeds": add_text_embeds, "time_ids": add_time_ids, \
                            "control_type":union_control_type_.reshape(1, -1).to(device, dtype=prompt_embeds.dtype).repeat(batch_size * num_images_per_prompt * 2, 1)})
 
                    if layout_type is not None:
                        layout_type_ = layout_type.reshape(1, -1).to(device, dtype=prompt_embeds.dtype).repeat(batch_size * num_images_per_prompt * 2, 1)
                ...
                if isinstance(controlnet, ControlNetModel_Union):
                    ...
                    # print(len(down_block_res_samples), len(mid_block_res_sample))
                    noise_pred = self.unet(
                        latent_model_input,
                        t,
                        encoder_hidden_states=prompt_embeds,
